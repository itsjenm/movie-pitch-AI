...
// function for the bot reply that is outputted once it receives a request
...
// function that generates a synopsis
...
// function that generates movie title by using the synopsis
async function fetchTitle(synopsis) {
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `Generate a catchy movie title for this synopsis: ${synopsis}`,
    max_tokens: 25,
    // temperature = controls how often the model outputs a less likely token
    // temperature setting defaults to 1. from 0 to 1 increments of .01
    temperature: 0.6,
  });
...
// function that lists the names of the actors in the movie. 
async function fetchStars(synopsis){
  const response = await openai.createCompletion({
    ...
    ###
    synopsis: The Top Gun Naval Fighter Weapons School is where the best of the best train to refine their elite flying skills. When hotshot fighter pilot Maverick (Tom Cruise) is sent to the school, his reckless attitude and cocky demeanor put him at odds with the other pilots, especially the cool and collected Iceman (Val Kilmer). But Maverick isn't only competing to be the top fighter pilot, he's also fighting for the attention of his beautiful flight instructor, Charlotte Blackwood (Kelly McGillis). Maverick gradually earns the respect of his instructors and peers - and also the love of Charlotte, but struggles to balance his personal and professional life. As the pilots prepare for a mission against a foreign enemy, Maverick must confront his own demons and overcome the tragedies rooted deep in his past to become the best fighter pilot and return from the mission triumphant.
    names: Tom Cruis, Val Kilmer, Kelly McGillis
    ###
    synopsis: ${synopsis}
    names: 
    `,
...
async function fetchImagePrompt(title, synopsis) {
  ...
  fetchImageUrl(imageDescription)
  
}

// use await openai.createImage({}) to generate an image
// needs prompt(description of image), n (number of images generated, btwn 1 and 10, defaults to 1), size (in pixels, defaults to 1024x1024, bigger images cost more credits), response_format(string 'url' or 'b64_json', defaults to 'url')
// openAI image URLS only last one hour, you need to download 
// b64_json can be decoded to an image, and does not expire 
...
// 1. A pizza delivery rider infiltrates an organized crime group. 